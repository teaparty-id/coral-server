<template>
  <section class="py-20 bg-base-200">
    <div class="container mx-auto px-6">
      <div class="text-center">
        <p class="text-sm uppercase tracking-widest opacity-60">Dibangun Untuk Pemilik Rental PlayStation Modern</p>

        <h2 class="text-4xl font-bold mt-4">Satu Platform Untuk Seluruh Operasional Rental</h2>
      </div>

      <div class="grid md:grid-cols-4 gap-6 mt-12">
        <div class="stat bg-base-100 rounded-box shadow text-current">
          <div class="stat-value">24/7</div>
          <div class="stat-title">Monitoring</div>
        </div>

        <div class="stat bg-base-100 rounded-box shadow text-current">
          <div class="stat-value">∞</div>
          <div class="stat-title">Multi Cabang</div>
        </div>

        <div class="stat bg-base-100 rounded-box shadow text-current">
          <div class="stat-value">0</div>
          <div class="stat-title">Relay Listrik</div>
        </div>

        <div class="stat bg-base-100 rounded-box shadow text-current">
          <div class="stat-value">100%</div>
          <div class="stat-title">Terintegrasi</div>
        </div>
      </div>
    </div>
  </section>
</template>
